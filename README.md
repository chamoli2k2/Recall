...
`server/src/realtime/rooms.js` is a small game server: a `RoomStore` of in-memory rooms keyed by a six-character code (alphabet without 0/O/1/I), each holding the shuffled question list, the players' scores and streaks, the current phase (`lobby → question → reveal → … → finished`) and the question deadline. Everything is **server-authoritative**: clients only send intents (`room:create`, `room:join`, `room:start`, `room:answer`, `room:next`) and receive a whole-state `room:state` snapshot after every change. Snapshots are rendered per socket so the answer key and each player's verdict are withheld while a question is open — you cannot learn the answer by reading the WebSocket traffic.

Questions are built once at creation: every card with an answer becomes a prompt with its own answer plus up to three distinct distractors sampled from the other cards' answers (cloze cards use the blanked sentence as the prompt and the hidden words as the answer). Scoring is `1000 × (0.5 + 0.5 × remaining/total)` for a correct answer, plus `100 × (streak − 1)` capped at 500; wrong or missed answers reset the streak. A server-side timer reveals when the clock runs out, and the round reveals early as soon as every connected player has answered. Hosts need read access to the folder; players only need to be signed in, so a private folder can still be played with friends who cannot see it. If the host disconnects mid-game the role passes to a connected player; a room is destroyed when its last socket leaves. The store takes an injectable clock and scheduler, which is how `server/test/rooms.test.js` asserts timeouts, early reveals and host handover deterministically, and `scripts/quiz-peer.js` is a bot that joins a room and answers, for trying rooms alone.

Trade-off: rooms live in one process, so a multi-instance deployment would need sticky sessions or a Redis-backed store; that is acceptable for a feature whose state is worthless after the game ends.
