...
export const optionalAuth = asyncHandler(async (req, _res, next) => {
  if (req.cookies.recall_session) {
    const session = await Session.findOne({ tokenHash: hashToken(req.cookies.recall_session), expiresAt: { $gt: new Date() } });
...
export async function createSession(res, user) {
  ...
  res.cookie('recall_session', token, cookieOptions());
}
