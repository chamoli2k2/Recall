...
const premiumOrder = new Schema({ user: ref('User'), plan: { type: String, required: true }, name: { type: String, required: true }, email: { type: String, required: true }, phone: { type: String, required: true }, country: { type: String, required: true }, address: { type: String, required: true }, proof: { type: Buffer, required: true, select: false }, proofType: { type: String, default: 'image/webp' }, status: { type: String, enum: ['pending', 'approved', 'declined'], default: 'pending' }, reviewedBy: ref('User', false) }, options);
...
export const allModels = [User, Session, Folder, Card, Media, Progress, Review, Activity, Revision, DomainEvent, CardDoc, Relationship, Project, PremiumOrder, Notification];
