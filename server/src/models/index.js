...
const progress = new Schema({ user: ref('User'), card: ref('Card'), repetitions: { type: Number, default: 0 }, interval: { type: Number, default: 0 }, ease: { type: Number, default: 2.5 }, dueAt: { type: Date, default: Date.now }, lastReviewedAt: Date, version: { type: Number, default: 0 }, bookmarked: { type: Boolean, default: false } }, options);
progress.index({ user: 1, card: 1 }, { unique: true });
const review = new Schema({ user: ref('User'), card: ref('Card'), requestId: { type: String, required: true }, rating: { type: String, enum: ['again', 'hard', 'good', 'easy'] }, result: Schema.Types.Mixed }, options);
review.index({ user: 1, requestId: 1 }, { unique: true });
