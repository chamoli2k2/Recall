...
const premiumOrder = new Schema({ user: ref('User'), name: { type: String, required: true }, email: { type: String, required: true }, phone: { type: String, required: true }, country: { type: String, required: true }, address: { type: String, required: true }, proof: { type: Buffer, required: true, select: false }, proofType: { type: String, default: 'image/webp' }, status: { type: String, enum: ['pending', 'approved', 'declined'], default: 'pending' }, reviewedBy: ref('User', false) }, options);
premiumOrder.index({ user: 1, createdAt: -1 });
export const User = model('User', user), Session = model('Session', session), Folder = model('Folder', folder), Card = model('Card', card), Media = model('Media', media), Progress = model('Progress', progress), Review = model('Review', review), Activity = model('Activity', activity), Revision = model('Revision', revision), DomainEvent = model('DomainEvent', event), CardDoc = model('CardDoc', cardDoc), Relationship = model('Relationship', relationship), Project = model('Project', project), PremiumOrder = model('PremiumOrder', premiumOrder);
export const allModels = [User, Session, Folder, Card, Media, Progress, Review, Activity, Revision, DomainEvent, CardDoc, Relationship, Project, PremiumOrder];
