#!/usr/bin/env node
// Standalone external health ping for Recall. Runs once and exits; intended for an external scheduler
// (cron, systemd timer, GitHub Actions, or a hosted monitor) on a machine independent of the Recall server.
// Usage: HEALTHCHECK_URL=https://recall.example.com/api/health node scripts/healthcheck.js
...
export async function checkOnce(url, { fetchImpl = fetch, timeoutMs = DEFAULTS.timeoutMs } = {}) {
  ...
  try {
    const res = await fetchImpl(url, { signal: controller.signal, redirect: 'manual', headers: { accept: 'application/json', 'user-agent': 'recall-healthcheck/1.0' } });
...
export async function run({ url = process.env.HEALTHCHECK_URL, fetchImpl = fetch, timeoutMs = DEFAULTS.timeoutMs, retries = DEFAULTS.retries, logger = defaultLogger } = {}) {
  if (!url || !/^https?:\/\//.test(url)) { logger('error', 'HEALTHCHECK_URL must be set to an http(s) URL, e.g. https://recall.example.com/api/health'); return 2; }
