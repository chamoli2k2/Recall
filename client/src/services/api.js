...
export async function api(path, options = {}) {
  ...
  try {
    ...
    throw new ApiError('Recall could not reach the server. Check your connection and try again.', { code: 'NETWORK', details: { cause: cause?.message } });
  }
  const requestId = response.headers.get('x-request-id');
  const type = response.headers.get('content-type') || '';
  if (!type.includes('application/json')) {
    throw new ApiError('The Recall API is unavailable. Start the Express server and check your database connection.', { status: response.status, code: 'NOT_JSON', requestId });
  }
